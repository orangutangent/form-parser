...
const Input: React.FC<Props> = ({
	...
	const handleChange = (event: React.ChangeEvent<HTMLInputElement>) => {
		...
		if (regExp && !regExp.test(inputValue)) {
			setError(`Invalid password format ${mask && '(' + mask + ')'}`);
		} else if (maxLength && inputValue.length > maxLength) {
			setError(`Password length should be less than ${maxLength}`);
		} else if (minLength && inputValue.length < minLength) {
			setError(`Password length should be greater than ${minLength}`);
		} else {
			setError('');
		}
	};
